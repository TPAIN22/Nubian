...
import { getFinalPrice, getOriginalPrice, hasDiscount } from "@/utils/priceUtils";
...
const ProductCard = React.memo(
  ({ item, onPress, variant = "grid", showWishlist = true, cardWidth: providedCardWidth }: ProductCardProps) => {
    ...
    const validImages = useMemo(() => cleanImages(item.images), [item.images]);
    const singleImage = validImages.length === 1 ? validImages[0] : null;

    // ✅ pricing: for cards use lowest eligible variant (or fallback to product)
