...
const AddToCartButton = ({
  ...
  const matchingVariant = useMemo(() => {
    ...
    return findMatchingVariant(product, mergedAttributes);
  ...
  const availability = useMemo(() => {
    ...
    if (!hasVariants) {
      ...
      return stock > 0 ? { ok: true, reason: "ok" as const } : { ok: false, reason: "out_of_stock" as const };
    }
    ...
    // variant inactive
    if (matchingVariant.isActive === false) return { ok: false, reason: "inactive_variant" as const };

    // stock
    const vStock = matchingVariant.stock ?? 0;
    if (vStock <= 0) return { ok: false, reason: "out_of_stock" as const };
  ...
  const handleAddToCart = useCallback(async () => {
    ...
    try {
      ...
      if (!isSignedIn) {
        ...
        Toast.show({ type: "error", text1: i18n.t("pleaseSignInFirst") || "Please sign in first" });
        return;
      }

      // ✅ أهم سطر: ابعت نفس attributes اللي اخترتها
      await addToCart(product._id, 1, mergedAttributes.size || "", mergedAttributes);
      ...
      Toast.show({ type: "success", text1: i18n.t("addedToCart") || "Added to cart" });
    } catch (err: any) {
      const msg = err?.response?.data?.message || err?.message || i18n.t("addToCartError") || "Error";
      Toast.show({ type: "error", text1: i18n.t("addToCartError") || "Add to cart error", text2: String(msg) });
    } finally {
      setIsLoading(false);
    }
  }, [
